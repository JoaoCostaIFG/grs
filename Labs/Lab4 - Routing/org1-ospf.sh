...
sudo docker build --tag quagga ~/quagga
...
sudo docker run -d --net org1_int1_net --ip 10.0.1.2 \
    ...
    -v "$(pwd)"/quagga/start.sh:/root/start.sh \
    --cap-add=NET_ADMIN  --privileged --name org1_router1 quagga 
...
# org1 router 2
echo "Deploying router 2"
sudo docker run -d --net org1_int2_net --ip 10.0.1.11 \
    -v "$(pwd)"/quagga/zebra.conf:/etc/quagga/zebra.conf \
    -v "$(pwd)"/quagga/ospfd.conf:/etc/quagga/ospfd.conf \
    -v "$(pwd)"/quagga/start.sh:/root/start.sh \
    --cap-add=NET_ADMIN --privileged --name org1_router2 quagga 
...
# org1 router 3
echo "Deploying router 3"
sudo docker run -d --net org1_int1_net --ip 10.0.1.3 \
    -v "$(pwd)"/quagga/zebra.conf:/etc/quagga/zebra.conf \
    -v "$(pwd)"/quagga/ospfd.conf:/etc/quagga/ospfd.conf \
    -v "$(pwd)"/quagga/start.sh:/root/start.sh \
    --cap-add=NET_ADMIN --privileged --name org1_router3 quagga 
